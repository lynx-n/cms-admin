...
//            }
        ...
        resource.setIcon(host + Constant.CMS_STORAGE.getValue() + Constant.IMAGE_ICON_PATH.getValue() + resource.getUuid() + "/" + iconId);
        log.info("SAVE THUMBNAIL URL IS : {}", resource.getIcon());


        // 4-存储实体数据
        String fileId = saveToStore(file, resource);
        resource.setFilePath(host + Constant.CMS_STORAGE.getValue() + Constant.IMAGE_FILE_PATH.getValue() + resource.getUuid() + "/" + fileId);
